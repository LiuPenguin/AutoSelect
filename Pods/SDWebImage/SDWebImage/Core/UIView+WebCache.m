...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    if (context) {
        ...
        context = [NSDictionary dictionary];//初始化一个字典 放啥一会儿就知道
    }
    NSString *validOperationKey = context[SDWebImageContextSetImageOperationKey];//找到这个key
    if (!validOperationKey) {//找不到就是新初始化了一个字典
        // pass through the operation key to downstream, which can used for tracing operation or image view class
        validOperationKey = NSStringFromClass([self class]);//当前类名
        SDWebImageMutableContext *mutableContext = [context mutableCopy];//可变字典
        mutableContext[SDWebImageContextSetImageOperationKey] = validOperationKey;//类名为键
    ...
    //context
//    {
//        setImageOperationKey = UIImageView;
//    }
